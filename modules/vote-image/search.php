...
$db->sqlreset()
    ...
    ->where('active=1 AND (' . nv_like_logic('fullname', $dbkeyword, $logic) . ' OR ' . nv_like_logic('email', $dbkeyword, $logic) .')');
...
if ($num_items) {
    ...
    while ($row = $result->fetch()) {
        $result_array[] = [
            ...
            'content' => BoldKeywordInStr($row['fullname'] . ' ' . $row['email'], $key, $logic)
        ];
    }
}
