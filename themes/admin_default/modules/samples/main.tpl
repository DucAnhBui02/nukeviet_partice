...
<!-- END: error -->
    <form action="{NV_BASE_ADMINURL}index.php?{NV_LANG_VARIABLE}={NV_LANG_DATA}&amp;{NV_NAME_VARIABLE}={MODULE_NAME}&amp;{NV_OP_VARIABLE}={OP}" method="post">
        ...
        <div class="form-group row">
            ...
            <div class="col-md-20">
            <!-- BEGIN: gender -->
                <input type="radio" class="form-control" name="gender" value="{GENDER.key}" {GENDER.checked}/>{GENDER.title}
            <!-- END: gender -->
            </div>
        </div>
        <div class="form-group row">
    ...
    <script type="text/javascript">
        function changeProvide() {
            ...
            $.ajax({url: script_name + '?' + nv_lang_variable + '=' + nv_lang_data + '&' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=main&change_provide=1&id_provide',  + id_provide, success: function(result){
                if(result != "ERR")
